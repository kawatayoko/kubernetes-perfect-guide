...
- WorkloadAPIのまとめ
    ...
    - StatefulSet(Tier2)
        ...
        - Pod
# 第６章 Servive APIsカテゴリ
- Workloads APIs カテゴリ
    - コンテナの実行に関するリソース
- Service APIs カテゴリ
    - コンテナを外部公開するようなエンドポイントを提供するリソース
        - L4 ロードバランシングを提供する `Service` リソース
        - L7 ロードバランシングを提供する `Ingress` リソース
- Config & Storage APIs カテゴリ
    - 設定/機密情報/永続化ボリュームに関するリソース
## 6.2 KubernetesクラスタのネットワークとService
- 同じPodのコンテナは全て同じIPアドレスが割り当てられる
- Kubernetesクラスタ  
    - ノードにPodのための内部ネットワークを構築する
    - 内部ネットワークの構成
        - ノード毎に異なるネットワークセグメント
            - Kubernetesクラスタ全体に割り当てられたネットワークセグメントを自動的に分割して割り当てる
        - ノード間のトラフィックは以下を利用
            - VXLANや
            - L2 Routing
    - 内部ネットワークが自動構成されているので、PodはServiceを利用せずともPod間通信を行える
        - しかしServiceを利用するメリットがある
            - Pod宛トラフィックのロードバランシング
            - サービスディスカばりとクラスタ内DNS
- Serviceは受信したトラフィックを複数のPodにロードバランシングする機能を提供する
    - Deploymentを使用してPodを複数起動できるが、Podにはそえぞれ異なるIPアドレスが割り当てられている。
    - そのためロードバランシグをする仕組を独自に実装するのは大変
    - Serviceはロードバランシングの接続口となるエンドポイントも提供する
    - エンドポイントの種類
        - VirtualIPアドレス
            外部のロードバランさが払い出す仮想IPアドレス
        - ClusterIP
            クラスタ内のみで利用可能な仮想IPアドレス
    - 一つのServiceに複数ポート割り当て可能
        - Serviceで8080で受けたもの -> Podの80ポートへ、みたいなことが可能
    - 名前によるポートの参照も可能
        - Pod側のマニフェストでコンテナポート80,81にhttpと命名
        - Service側のマニエストで`targetPort:http`と記載可能
- クラスタ内DNSとサービスディスカばり
    - DNSを利用したサービスディスカバリではクラスタ内臓のDNSサーバ（クラスタ内DNS）へ自動登録されるServiceのエンドポイント情報を利用する
        - 環境変数を利用したサービスディスカバリ
            - Pod内からは環境変数でも同じNamespaceのサービスが確認できる
        - DNS A レコードを利用したサービスディスカバリ
            - 他のPodからServiceで払い出されるエンドポイントへ接続するには宛先が必要
                - 自動登録されたDNSレコードを使用できる
            - KubernetesではIPアドレスの管理から解放されるために基本的に自動的に払い出されたIPアドレスに紐づくDNS名を登録するのが望ましい
                - 払い出されるIPアドレスはService作成のたびに変わる
